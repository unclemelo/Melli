...
class Credits(commands.Cog):
    ...
    async def creditscmd(self, interaction: discord.Interaction):
        try:            
            ...
            embed.add_field(name="Developers", value="`@_uncle_melo_`, `@pitr1010`, `@illtana`", inline=True)
            embed.add_field(name="Editor", value="`@mizuki_mochizuki2090`", inline=True)
            embed.add_field(name="Artists", value="`@soupinascoop`, `@bunnnl`", inline=True)
